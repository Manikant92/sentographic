...
def get_tweet_count_over_time_graph(dataframe):
    ...
    graph.update_layout(
        autosize=True, xaxis_title='Date & Time', yaxis_title='Tweet Count')
...
def get_news_count_over_time_graph(dataframe):
    print('Creating News over time graph')
    date_frame = pd.DataFrame(columns=['date', 'count'])
    date_time_data = dataframe['date'].value_counts()
    for key, value in date_time_data.items():
        new_dict = {'date': key, 'count': value}
        date_frame = date_frame.append(new_dict, ignore_index=True)
    date_frame = date_frame.sort_values(by=['date'], ascending=True)
    graph = px.line(date_frame, x='date', y='count')
    graph.update_layout(
        autosize=True, xaxis_title='Date', yaxis_title='News Count')
    graph.update_traces(mode="markers+lines")
    plot_div = plot(graph, output_type='div', config=config)
    print('Created News Over time graph')
    return plot_div
...
def get_news_wordcloud(dataframe):
    print('Generating tweet Wordcloud')
    tweet_words = ' '.join([tweets for tweets in dataframe['content']])
    no_urls_no_tags = " ".join(
        [word for word in tweet_words.split() if 'http' not in word and not word.startswith('@') and word != 'RT'])
    tweet_wordcloud = WordCloud(stopwords=STOPWORDS, background_color='white', width=1800, height=1400,
                                max_words=50).generate(no_urls_no_tags)
...
def get_entity_cloud(dataframe):
    hashtag_dict = {}
    tweet_hashtag_list = []
    for hashtag_list in dataframe['entity']:
        if not isinstance(hashtag_list, list):
            hashtag_list = ast.literal_eval(hashtag_list)
        if len(hashtag_list) > 0:
            for hashtag in hashtag_list:
                tweet_hashtag_list.append(hashtag)
    hashtag_df = pd.DataFrame({'entity': tweet_hashtag_list})
    hashtags = dict(hashtag_df['entity'].value_counts())
    hashtag_sorted_dict = sorted(
        hashtags.items(), key=lambda x: x[1], reverse=True)
    for hashtag, value in hashtag_sorted_dict:
        hashtag_dict[hashtag] = value
...
def get_mainlemma_cloud(dataframe):
    hashtag_dict = {}
    tweet_hashtag_list = []
    for hashtag_list in dataframe['lemma']:
        if not isinstance(hashtag_list, list):
            hashtag_list = ast.literal_eval(hashtag_list)
        if len(hashtag_list) > 0:
            for hashtag in hashtag_list:
                tweet_hashtag_list.append(hashtag)
    hashtag_df = pd.DataFrame({'lemma': tweet_hashtag_list})
    hashtags = dict(hashtag_df['lemma'].value_counts())
    hashtag_sorted_dict = sorted(
        hashtags.items(), key=lambda x: x[1], reverse=True)
    for hashtag, value in hashtag_sorted_dict:
        hashtag_dict[hashtag] = value
...
def get_top_tweeters(dataframe):
    ...
    tweeter_sorted_dict = sorted(
        tweeter.items(), key=lambda x: x[1], reverse=True)
...
def get_top_news_author(dataframe):
    tweeter_dict = {}
    tweeter = dict(dataframe['author'].value_counts())
    tweeter_sorted_dict = sorted(
        tweeter.items(), key=lambda x: x[1], reverse=True)
    for tweeter, value in tweeter_sorted_dict:
        tweeter_dict[tweeter] = value
...
def get_geomap(dataframe):
    ...
    mapdata = pd.merge(dataframe, geolocated, how='inner',
...
def get_sentiment_timeline(dataframe):
    new_dataframe = dataframe[['tweet_text',
                               'screenname', 'sentiment', 'date_time']]
    ...
    graph.update_layout(
        autosize=True, xaxis_title='Date & Time', yaxis_title='Sentiment')
    graph.update_traces(mode="markers+lines")
    graph.update_layout(hoverlabel=dict(
        bgcolor="white", font_size=16, font_family="Rockwell"))
    plot_div = plot(graph, output_type='div', config=config)
    return plot_div


def get_news_sentiment_timeline(dataframe):
    new_dataframe = dataframe[['content',
                               'author', 'sentiment', 'time']]
    date_frame = new_dataframe.sort_values(by=['time'], ascending=True)
    graph = px.line(date_frame, x='time', y='sentiment', hover_name="author",
                    hover_data=['content', 'time', 'sentiment'])
    graph.update_layout(
        autosize=True, xaxis_title='Time', yaxis_title='Sentiment')
    graph.update_traces(mode="markers+lines")
    graph.update_layout(hoverlabel=dict(
        bgcolor="white", font_size=16, font_family="Rockwell"))
    plot_div = plot(graph, output_type='div', config=config)
