...
def get_dataframe_row_values(tweet_json_data):
    ...
    try:
        ...
        tweet_dict['hashtags'] = list(
            ['#' + hashtag['text'] for hashtag in tweet_json_data['entities']['hashtags']])
        ...
        tweet_dict['source'] = re.findall(
            r'<a[^>]*>(.*?)</a>', tweet_json_data['source'])[0]
...
def analyze_sentiment(tweet):
    senti = expert_api.get_sentiment(tweet_txt=tweet)
